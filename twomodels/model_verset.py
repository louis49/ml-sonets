...
class VersetModel():
    ...
    def model(self, hp):
        ...
        learning_rate = 0.003 #hp.Float('learning_rate', min_value=0.00001, max_value=0.01, sampling='log', default=0.001) #1e-5 #1e-2
        ...
        #encoder_output = layers.LSTM(4 * lstm_units,
        ...
        # dropout=0.25 ??
        decoder_output = layers.Bidirectional(layers.LSTM(4 * lstm_units, return_sequences=True, return_state=False), name="decoder_output_BI_LSTM")(
        ...
        if attention_decoder:
            attention = layers.MultiHeadAttention(num_heads=num_heads_decoder, key_dim=max((4*lstm_units) // num_heads_decoder, 1), name="output_attention")
        ...
        if lstm_layer_decoder:
            #layers.Bidirectional( => A tester mais ne semble rien apporter de plus
            decoder_output = layers.LSTM(4 * lstm_units,
                                         return_sequences=True,
    ...
    def train(self, use_tuner=False, name=""):
        ...
        best_hyperparameters.Fixed('verset_lstm_units', value=16) #16
        ...
        best_hyperparameters.Fixed('drop_out_decoder_output', value=0.25) #0.25
        ...
        def scheduler(epoch, lr):
            ...
            if epoch < 100:
                ...
                learning_rate = lr * 0.8 #=> 0.004
            elif epoch == 200:
                learning_rate = lr * 0.5 #=> 0.002
            elif epoch == 261:
                learning_rate = lr * 0.5 #=> 0.001
            elif epoch == 300:
                learning_rate = lr * 0.5 #=> 0.0005
            elif epoch > 300:
