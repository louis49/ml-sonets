...
class VersetGenerator(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        if epoch % self.every_epoch == 0:
            ...
            text_2 = "Amour fou"
            generated_text_1 = self.generate_text(text_1, 'ʁ a s') #Ex : (g)râce
            generated_text_2 = self.generate_text(text_2, 'o t e') #Ex : (b)eauté
            print("\nSample generation at epoch {}: \n{} \n{}".format(epoch + 1, text_1 + ' ' + generated_text_1, text_2 + ' ' + generated_text_2))

    def generate_text(self, input_title, input_phons):
        ...
        one_hot_phons = tf.keras.utils.to_categorical(tokenized_phons, num_classes=self.data.phon_words)

        seed_text = '<start>'
        output_sequence = self.data.text_tokenizer.texts_to_sequences([seed_text])[0]

        for i in range(self.data.text_max_size):
            ...
            input_data = [np.array(padded_title), one_hot_phons[np.newaxis, ...], np.array(current_sequence)]
